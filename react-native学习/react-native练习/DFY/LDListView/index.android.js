...
class LDListView extends Component {
    ...
    fetchData2 () {
        ...
        for(let i = 0;i < organizations.length;i++){
            ...
            b[organization.id]=organization.organization;
            let users = organization.users;
            rowIDs[i] = [];
            for(let j = 0;j<users.length;j++){
                let user = users[j].user;
                rowIDs[i].push(user.md5);
                b[organization.id+user.md5] = user;
            }
        }
        this.setState({
            dataSource : this.state.dataSource.cloneWithRowsAndSections(b, sectionIDs, rowIDs),
            loaded     : true
        });
...
AppRegistry.registerComponent('LDListView', () => LDListView);
